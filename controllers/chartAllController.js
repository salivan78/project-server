...
class chartController {
    ...
    async createAllDepartment(req, res) {
        try {
            const {year} = req.body;
            const departmentName = await Department.findOne({name: 'all'})

            if (!departmentName) {
                const department = new Department({name: 'all', nameRus: 'Общая'})
                ...
                return res.json({message: `Подразделение all с именем Общая и годом ${year} добавлено`})
            } else {
                const dep = await Department.findOne({name: 'all'})
                ...
                if (!yearCandidate) {
                    ...
                    return res.status(400).json({message: `Year ${year} already exist`})
                }
            }
        } catch (e) {
            console.log(e)
    ...
    async getChartAll(req, res) {
        try {
            ...
            const odp = await Department.findOne({name: 'odp'})
            const prevHoursODP = await Chart.findOne({department: odp._id, year: prevYear})
            const so = await Department.findOne({name: 'so'})
            const prevHoursSO = await Chart.findOne({department: so._id, year: prevYear})
            const osfif = await Department.findOne({name: 'osfif'})
            const prevHoursOSFIF = await Chart.findOne({department: osfif._id, year: prevYear})
            const okp = await Department.findOne({name: 'okp'})
            const prevHoursOKP = await Chart.findOne({department: okp._id, year: prevYear})
            const opf = await Department.findOne({name: 'opf'})
            const prevHoursOPF = await Chart.findOne({department: opf._id, year: prevYear})
            const ohp = await Department.findOne({name: 'ohp'})
            const prevHoursOHP = await Chart.findOne({department: ohp._id, year: prevYear})
            const ops = await Department.findOne({name: 'ops'})
            const prevHoursOPS = await Chart.findOne({department: ops._id, year: prevYear})
